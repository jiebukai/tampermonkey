...
**清理策略**（**已定：365 天 + 上限 5000 条**，沿用既有模式）
...
- **已定：本地记录命中时仍然推送**，只在 toast 里说明「已推送过」，不阻断（用户手动重推通常是有意的，比如换了目标文件夹）。
- **两者互补，默认都开**：
...
## 9. 已定项（2026-09-21 用户确认）

| 项 | 结论 |
|---|---|
| 记录保留策略 | **365 天 + 上限 5000 条**（超出按 `pushedAt` 最旧淘汰）|
| 徽标文案 | **`已推送`**；重复推送时 **`已推送 ×N`**（文件夹名与推送时间放 `title` 悬停显示）|
| 本地记录命中的处理 | **仍然推送**，只在 toast 里说明「已推送过」，不阻断 |
