...
| 脚本 | 版本 | 适用站点 | 说明 |
| --- | --- | --- | --- |
| `抖音推送eagle.user.js` | 1.2.0 | `https://*.douyin.com/*` | 作品详情页的播放器「插件」菜单、作者主页右下角浮动面板：把当前作品（视频或整套图集）推送到 Eagle，支持勾选多个作品批量推送；可按作者名归类（追加为标签，或在目标文件夹下自动建同名子文件夹）；动图（实况）格式可选高清图（webp）/ 动图（mp4）/ 两者都要；保留上游的下载能力 |
